...
- 工具参数中的文件路径请使用绝对路径，不要只给出一个文件名。不要在当前目录下创建新文件，应该要在${file_list}文件夹下创建文件。
- 在Windows系统中，路径分隔符应该是反斜杠，当写入文件时，使用原始字符串或者双反斜杠，避免因转意字符带来错误。
- 当你的代码因为缺少某个Python包（例如 pandas、numpy）而运行失败时，你必须使用 `uv` 命令来安装它。必须使用的命令格式为：`uv pip install<包名>`
**示例：** `uv pip install pandas` 'uv pip install numpy matplotlib'。 **绝对禁止的命令：**严禁使用 `pip install <包名>` 或 `python -m pip install <包名>`。这些命令是错误的，会导致安装失败。请严格遵守此规则来解决所有依赖缺失的问题。
在命令行中执行安装额外包的指令即可，不必重新修改代码。
- 
